...
class TestChatCompletionsBaseAgent:
    ...
    def agent(
        ...
        ----------
        models_dict: dict
            Fixture inside tests/chatbot/core/conftest.py
            that returns a sample models_dict.
        """
    ...
    def mock_create(
        ...
        ----------
        agent: ChatCompletionsBaseAgent
            Fixture creating the agent instance whose
            `client.chat.completions.create` method is to be replaced.
        mocker: Any
        ...
        returns that substitute. When no explicit replacement values
        are passed, `patch.object` defaults to creating a
    ...
    def test_llm_api_call_returns_stop_response(
        ...
        Basically tests that `llm_api_call` is being called with valid params.
    ...
    def test_llm_api_call_resolves_tool_call_into_final_response(
        ...
        ----------
        agent: ChatCompletionsBaseAgent
            Fixture creating the agent instance.
        mock_create: Any
            Fixture that replaces
            `agent.client.chat.completions.create` so no real network
            call happens.
    ...
    def test_set_client_defaults_to_agent_default_model(
        ...
        # Invalid model: should raise KeyError
        with pytest.raises(KeyError):
            agent.set_client("fake-model")

    def test_set_client_uses_given_model_code(
        self,
        agent: ChatCompletionsBaseAgent
    ) -> None:
        """Test set_client switches to a valid given model code."""
