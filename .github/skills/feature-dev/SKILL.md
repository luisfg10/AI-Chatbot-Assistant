...
- **Test thoroughly**: Ensure all new code has appropriate test coverage and passes linters  
...
1. **Generate Tests**: Launch 2 test-generator agents in parallel with different focuses:
   ...
   Tests need to go inside the `tests/` directory, with different files for each distinct test case.
...
4. **Run Tests**: Launch test-runner agent to:
   - Execute the full test suite (or relevant subset) using `uv run pytest {suite or subset}`
...
**Actions**:
1. Run `uv run ruff check .` for an initial code scan.  
2. If the previous step returned flags, fix the errors and return to step 1.  
3. Launch 2 code-reviewer agents in parallel with different focuses: simplicity/DRY/elegance, bugs/functional correctness, project conventions/abstractions.  
